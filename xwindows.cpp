...
#define Error(c) cout << "Error: " << (c) << endl;
...
XWindow::XWindow (void):
    xcorner(130),
    ycorner(100),
    tmp("/tmp/X11MovieFrames") // temporary path for movie frames
{ 
...
//If Ubuntu misses some fonts, they can be found in the file /usr/share/fonts/X11/Type1/fonts.dir
//const char* fontName="-bitstream-courier 10 pitch-medium-r-normal--0-0-0-0-m-0-iso10646-1";
...
void XWindow::SetColor(int r, int g, int b) 
{
XSetForeground(display, gc, r | g | b);
}
...
switch (k%n)
	...
	case 4: Yellow(); break;
	case 5: Violet(); break;
	default: cout << "XWindow::SetColor (int k): internal error." << endl;
	}
}
...
if (strcasecmp(extension,".eps")==0) 
	...
	{
	cerr << "XWindow::SaveImage: Unknown extension " << extension << endl;
	return false;
	}
return true;
}
...
cout << "write image " << commandline << endl;
